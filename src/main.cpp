...
static void save_pugixml_small()
{
    ...
    doc.load_string(small_xml_src.c_str());
...
static void save_pugixml_medium()
{
    ...
    pugi::xml_document doc;
    doc.load_string(medium_xml_src.c_str());
    doc.save_file(file_path.c_str(), "    ");
}
...
static void save_pugixml_big()
{
    ...
    pugi::xml_document doc;
    doc.load_string(big_xml_src.c_str());
    doc.save_file(file_path.c_str(), "    ");
}
...
{
    // Лог не используется в программе, но используется в функциях движка
    dviglo::Log log(base_path + "log.txt");
